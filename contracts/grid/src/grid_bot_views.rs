...
impl GridBotContract {
    pub fn query_bot(&self, bot_id: String) -> GridBotOutput {
        require!(self.bot_map.contains_key(&bot_id), INVALID_BOT_ID);
        return self.internal_get_grid_bot_output(&self.bot_map.get(&bot_id).unwrap());
    }

    pub fn query_bots(&self, bot_ids: Vec<String>) -> Vec<GridBotOutput> {
        let mut grid_bots: Vec<GridBotOutput> = Vec::with_capacity(bot_ids.len());
