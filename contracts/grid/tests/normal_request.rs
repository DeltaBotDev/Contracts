...
//     // 5800064546200000-5800062407270000=2138930000
//     require!(user_usdc_balance.clone().0 - taker_usdc_balance_before.clone().0 == 2138930000 as u128);
...
//     require!(taker_eth_balance_before.clone().0 - user_eth_balance.clone().0 == 100000000 as u128);
//
//     // query global balance
//     let global_usdc = gridbot_contract.query_global_balance(usdc_token_contract.get_account_id()).await?.unwrap();
//     println!("global_usdc:{}", global_usdc.0.to_string());
...
//     // 2140000000 - 2140000000 * 500 / 1000000 = 2140000000-1070000
//     require!(gridbot_usdc_balance == U128::from(99997861070000 as u128));
...
async fn create_bot() -> Result<(), workspaces::error::Error> {
    ...
    // register pair
    // check_success(gridbot_contract.register_pair(&owner, &(eth_token_contract.get_account_id()), &(usdc_token_contract.get_account_id()), U256C::from(1000), U256C::from(1000), eth_oracle_id.to_string(), usdc_oracle_id.to_string()).await);
    // check_success(gridbot_contract.register_pair(&owner, &(AccountId::from_str("wrap.testnet").expect("")), &(usdc_token_contract.get_account_id()), U256C::from(1000), U256C::from(1000), eth_oracle_id.to_string(), usdc_oracle_id.to_string()).await);
    ...
    check_success(gridbot_contract.create_bot(&maker_account, pair_id.clone(), 8000, GridType::EqOffset, 0,
                                              ...
                                              U256C::from(3500000000000000000 as u128)).await);
    let next_bot_id = format!("GRID:{}", "2".to_string());
    // // query storage fee
